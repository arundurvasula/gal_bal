...
parser = add_option(parser,c("-g","--genome-width"),default=30000,dest="genome_width")
...
#tile_genome = GRanges(seqnames="chr1",IRanges(seq(1,genome_width-window_width+1,by=window_step),end=seq(window_width,genome_width,by=window_step)))
### Diversity all ## 
...
out_slope_summary_stats = paste(input_sim,".rds",sep="")
print(head(tile_df))
print(head(pi_between_df))
pi_between_df = as.data.frame(pi_between)
tile_df2 = tile_df %>% full_join(pi_between_df,by=c("query"="query"))
mutation_list = list(out_slopes=out_df, pi_between_df=tile_df2, mutation_df=mutation_df,site_frequency_spectrum=mac)
